...
function bizz_booking_validate() {
	...
	if (isset($_GET["step"])) {
		...
		elseif ($qs_step == "3") {	
			...
			$carhire_cookie->car_dealer_email_id = get_post_meta( $params["car_id"], 'bizzthemes_car_dealer_email', true );
		...
		// validating step 5 : user checkouts to payment --> read cookie
			...
			$bookopts['car_dealer_email_id'] = $carhire_cookie->car_dealer_email_id;
			...
			add_post_meta($this_post_id, 'bizzthemes_bookings_dealer_email_id', $bookopts['car_dealer_email_id']); #dealer_email_id
		...
		if ( isset( $carhire_cookie->coupon ) && $carhire_cookie->coupon != '' ) {
			...
			//if ( $opt_s['admin_notifications'] != 1 )
             //Changed KL  . wp mail not configured . using server mail
				mail($your_email, $subject, $body, $headers);
...
if($form_data['payment_method'] !='creditcard'){
			...
			if ($form_data['payment_method']== 'creditcard'){
		//mail('kl@blkom.dk','COOKIE' ,$carhire_cookie->redirect);
			//header('location: '.$qppaylink);
...
//KL using Quickpay instead of Mollie.
...
add_filter( 'bizz_payment_redirect', 'bizz_redirect_quickpay', 10, 5 );
...
$repl = array(".",",");
	...
	//reset mode
     ...
     // mail('kl@blkom.dk','link' ,json_encode($storder));
...
/* Check Quickpay response */

/*------------------------------------------------------------------*/

add_action( 'init', 'bizz_check_quickpay_response' );
...
switch ($qp_status) {
    ...
    default:
...

		{
			...
			mail("kl@blkom.dk", "API-problem", "API call failed: " . htmlspecialchars($e->getMessage()));
