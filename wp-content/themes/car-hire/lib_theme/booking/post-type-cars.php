...
function bizz_cars_edit_columns($columns){
	...
	$columns['bizz_car_dealer'] 		= __('Dealer ID', 'bizzthemes');
	$columns['bizz_car_dealer_email'] 	= __('Email', 'bizzthemes');
...
function bizz_cars_custom_columns($column){
	...
	//$car_dealer = $current_user->ID;

	switch ($column){
		case "bizz_car_type_select":
			if (isset($custom['bizzthemes_car_type'][0]) && $custom['bizzthemes_car_type'][0]){
		...
		break;
		*/  
		case "bizz_car_dealer": 
			$dealerid = $custom["bizzthemes_car_dealer"][0];
			if ($dealerid != '') { echo $dealerid; }
		break;
		case "bizz_car_dealer_email": 
			$dealeremail = $custom["bizzthemes_car_dealer_email"][0];
			if ($dealeremail != '') { echo $dealeremail; }
		break;
		case "bizz_car_stock":
...
/*------------------------------------------------------------------*/
add_filter("manage_edit-bizz_cars_sortable_columns", 'bizz_cars_columns_sort');
function bizz_cars_columns_sort($columns) {
	$custom = array(
		//'bizz_car_type_select'	=> 'bizz_car_type_select',
		'bizz_car_seats'		=> 'bizz_car_seats',
		'bizz_car_doors'		=> 'bizz_car_doors',
		'bizz_car_transmission'	=> 'bizz_car_transmission',
		'bizz_car_stock'		=> 'bizz_car_stock',
	);
	return wp_parse_args($custom, $columns);
}

add_filter( 'posts_clauses', 'manage_wp_cars_posts_be_qe_posts_clauses', 1, 2 );
function manage_wp_cars_posts_be_qe_posts_clauses( $pieces, $query ) {
	global $wpdb;
	...
	if ( $query->is_main_query() && ( $orderby = $query->get( 'orderby' ) ) ) {
		...
		switch( $orderby ) {
			/*
			case 'bizz_car_seats':
				$pieces[ 'join' ] .= " LEFT JOIN $wpdb->postmeta as wp_rd ON wp_rd.post_id = {$wpdb->posts}.ID AND wp_rd.meta_key = 'bizzthemes_car_seats'";
				$pieces[ 'orderby' ] = "wp_rd.meta_value $order, " . $pieces[ 'orderby' ];
			break;
			*/
			case 'bizz_car_doors':
				$pieces[ 'join' ] .= " LEFT JOIN $wpdb->postmeta as wp_rd ON wp_rd.post_id = {$wpdb->posts}.ID AND wp_rd.meta_key = 'bizzthemes_car_doors'";
				$pieces[ 'orderby' ] = "wp_rd.meta_value $order, " . $pieces[ 'orderby' ];
			break;
			case 'bizz_car_transmission':
				$pieces[ 'join' ] .= " LEFT JOIN $wpdb->postmeta as wp_rd ON wp_rd.post_id = {$wpdb->posts}.ID AND wp_rd.meta_key = 'bizzthemes_car_transmission'";
				$pieces[ 'orderby' ] = "wp_rd.meta_value $order, " . $pieces[ 'orderby' ];
			break;
			case 'bizz_car_stock':
				$pieces[ 'join' ] .= " LEFT JOIN $wpdb->postmeta as wp_rd ON wp_rd.post_id = {$wpdb->posts}.ID AND wp_rd.meta_key = 'bizzthemes_car_stock'";
				$pieces[ 'orderby' ] = "wp_rd.meta_value $order, " . $pieces[ 'orderby' ];
			break;
		}
	}
...
function bizz_cars_metaboxes( $meta_boxes ) {
	...
	foreach ($pricing_posts as $pricing_post) {
		if ($pricing_post->post_author==$current_user->ID) {
	...
	$locations_options = array();
	foreach ($locations_posts as $location_post) {
		if ($location_post->post_author==$current_user->ID) 
		{
	...
	$meta_boxes[] = array(
		...
		'fields' => array(
			...
			array(
				...
				'id' => $prefix . 'car_dealer',
				'type' => 'text_small',
				'std' => $current_user->ID	
			),
			array(
				'name' => __('Car dealer Email', 'bizzthemes'),
				'id' => $prefix . 'car_dealer_email',
				'type' => 'text_medium',
